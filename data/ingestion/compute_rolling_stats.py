...
def compute_todays_stats(window=10):
    """
    Force compute rolling stats as of today for all teams.
    Uses all available game data up to today.
    """
    ...
    today = date.today()
    print(f"Computing today's rolling stats ({today}, window={window})...")
    ...
    with Session(engine) as session:
        for team_id in teams:
            ...
            # Use last N games for today's stats
            if len(team_df) < window:
                continue

            window_df = team_df.tail(window)
            last_game_season = team_df.iloc[-1]['season']
            ...
            home_games = window_df[window_df['is_home'] == True]
            away_games = window_df[window_df['is_home'] == False]
            total_fga  = window_df['fg3a'].sum() + 0.001
            three_point_rate = float(
                window_df['fg3a'].sum() / total_fga
            )
            wins   = int((window_df['net_rating'] > 0).sum())
            losses = window - wins
            ...
            stats_data = dict(
                ...
                as_of_date           = today,
                window               = window,
                season               = last_game_season,
                avg_points           = float(window_df['points'].mean()),
                avg_offensive_rating = float(window_df['offensive_rating'].mean())
                                       if window_df['offensive_rating'].notna().any()
                                       else None,
                avg_defensive_rating = float(window_df['defensive_rating'].mean())
                                       if window_df['defensive_rating'].notna().any()
                                       else None,
                avg_pace             = float(window_df['pace'].mean())
                                       if window_df['pace'].notna().any()
                                       else None,
                avg_fg_pct           = float(window_df['fg_pct'].mean())
                                       if window_df['fg_pct'].notna().any()
                                       else None,
                avg_fg3_pct          = float(window_df['fg3_pct'].mean())
                                       if window_df['fg3_pct'].notna().any()
                                       else None,
                avg_ft_rate          = float(window_df['ft_rate'].mean())
                                       if window_df['ft_rate'].notna().any()
                                       else None,
                three_point_rate     = three_point_rate,
                home_avg_points      = float(home_games['points'].mean())
                                       if len(home_games) > 0 else None,
            ...
            if existing:
                ...
                session.add(TeamRollingStats(**stats_data))
                records_added += 1

        session.commit()

    print(f"✅ Today's rolling stats updated ({records_added} new records)")
...
if __name__ == "__main__":
    ...
    for window in [5,7, 10, 20]:
        ...
        compute_todays_stats(window=window)